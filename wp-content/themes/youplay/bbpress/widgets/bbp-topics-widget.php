...
class yp_BBP_Topics_Widget extends WP_Widget {
  ...
  public function __construct() {
    $widget_ops = apply_filters( 'bbp_topics_widget_options', array(
      ...
      'description' => __( 'A list of recent topics, sorted by popularity or freshness.', 'youplay' )
    ) );

    parent::__construct( false, __( '(bbPress) Recent Topics', 'youplay' ), $widget_ops );
  }
      ...
      <?php while ( $widget_query->have_posts() ) :
        ...
        <li>
          ...
          <?php if ( ! empty( $author_link ) ) : ?>
            ...
            <?php printf( _x( 'by %1$s', 'widgets', 'youplay' ), '<span class="topic-author">' . $author_link . '</span>' ); ?>
  ...
  public function form( $instance = array() ) {
    ...
    <p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php _e( 'Title:', 'youplay' ); ?> <input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr( $settings['title'] ); ?>" /></label></p>
    <p><label for="<?php echo esc_attr($this->get_field_id( 'max_shown' )); ?>"><?php _e( 'Maximum topics to show:', 'youplay' ); ?> <input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'max_shown' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'max_shown' )); ?>" type="text" value="<?php echo esc_attr( $settings['max_shown'] ); ?>" /></label></p>
    ...
    <p><label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php _e( 'Show post date:', 'youplay' ); ?> <input type="checkbox" id="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'show_date' )); ?>" <?php checked( true, $settings['show_date'] ); ?> value="1" /></label></p>
    <p><label for="<?php echo esc_attr($this->get_field_id( 'show_user' )); ?>"><?php _e( 'Show topic author:', 'youplay' ); ?> <input type="checkbox" id="<?php echo esc_attr($this->get_field_id( 'show_user' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'show_user' )); ?>" <?php checked( true, $settings['show_user'] ); ?> value="1" /></label></p>

    <p>
      <label for="<?php echo esc_attr($this->get_field_id( 'order_by' )); ?>"><?php _e( 'Order By:', 'youplay' ); ?></label>
      <select name="<?php echo esc_attr($this->get_field_name( 'order_by' )); ?>" id="<?php echo esc_attr($this->get_field_name( 'order_by' )); ?>">
        <option <?php selected( $settings['order_by'], 'newness' ); ?> value="newness"><?php _e( 'Newest Topics', 'youplay' ); ?></option>
        <option <?php selected( $settings['order_by'], 'popular' ); ?> value="popular"><?php _e( 'Popular Topics', 'youplay' ); ?></option>
        <option <?php selected( $settings['order_by'], 'freshness' ); ?> value="freshness"><?php _e( 'Topics With Recent Replies', 'youplay' ); ?></option>
      </select>
    </p>
  ...
  public function parse_settings( $instance = array() ) {
    return bbp_parse_args( $instance, array(
      'title'        => __( 'Recent Topics', 'youplay' ),
      'max_shown'    => 5,
      'show_date'    => false,
      'show_user'    => false,
      'parent_forum' => 'any',
