...
<form action="<?php echo trailingslashit( bp_displayed_user_domain() . bp_get_settings_slug() . '/profile' ); ?>" method="post" id="settings-form">
	...
	<?php if ( bp_xprofile_get_settings_fields() ) : ?>
		...
		<?php while ( bp_profile_groups() ) : bp_the_profile_group(); ?>
			...
			<?php if ( bp_profile_fields() ) : ?>
				...
				<table id="xprofile-settings-<?php bp_the_profile_group_slug(); ?>" class="table table-hover">
					<thead>
						<tr>
							<th class="title field-group-name"><?php bp_the_profile_group_name(); ?></th>
							<th class="title" width="250"><?php _e( 'Visibility', 'youplay' ); ?></th>
						</tr>
					</thead>
	...
	<div class="submit">
        <span class="btn btn-default">
            <?php esc_html_e( 'Save Settings', 'youplay' ); ?>
            <input type="submit" value="<?php esc_attr_e( 'Save Settings', 'youplay' ); ?>" id="submit" name="xprofile-settings-submit" />
        </span>
	</div>
